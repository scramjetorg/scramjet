...
class MultiStream extends EventEmitter {
     ...
     *
     * @see example in file: {@link ../samples/multi-stream-constructor.js}
     */
     ...
     *
     * @see example in file: {@link ../samples/multi-stream-map.js}
     */
    map(func) {
        return Promise.all(
            this[STREAMS].map(
     ...
     *
     * @see example in file: {@link ../samples/multi-stream-filter.js}
     */
    filter(func) {
        return Promise.all(
            this[STREAMS].map(
     ...
     *
     * @see example in file: {@link ../samples/multi-stream-dedupe.js}
     */
    dedupe(cmp, Clazz) {
        cmp = cmp || (() => {
     ...
     *
     * @see example in file: {@link ../samples/multi-stream-mux.js}
     */
    mux(cmp, Clazz) {
        if (Clazz) {
            this[OUT] = new Clazz();
     ...
     *
     * @see example in file: {@link ../samples/multi-stream-add.js}
     */
    add(stream) {
        if (stream) {
     ...
     * @param {stream.Readable} stream [description]
     *
     * @see example in file: {@link ../samples/multi-stream-remove.js}
     */
    remove(stream) {
