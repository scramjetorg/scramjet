...
class DataStream extends PassThrough {
    ...
    pop(count, func) {
        const ret = []; let cnt = 0;
        const oTransform = this.hasOwnProperty("_transform") && this._transform;

        const str = this.pipe(new DataStream());

        str._transform = (chunk, encoding, callback) => {
            ret.push(chunk);
            if (++cnt >= count) {
                func(ret);
                delete this._transform;
                if (oTransform) this._transform = oTransform;
            }
            callback();
        };
