...
class DataStream extends PassThrough {
    ...
    _mapFilter(arr) {
        const getFilteredPromise = (prm, func) => prm.then(
            (chunk) => Promise.resolve(chunk)
                .then(func)
                .then((out) => out ? chunk : Promise.reject(FILTERED))

        );
        const getMappedAndFilteredPromise = (chunk) => {
            let ref = Promise.resolve(chunk);
            for (var i = 0; i < arr.length; i++) {
                if (arr[i].filter)
                    ref = getFilteredPromise(ref, arr[i].func);
                else
                    ref = ref.then(arr[i].func);
            }
        };
        const nextStream = new DataStream({
            transform: (chunk, encoding, callback) => {
                return getMappedAndFilteredPromise(chunk)
                    .then(
                        (chunk) => callback(null, chunk)
                    )
                    .catch(
                        (error) => error === FILTERED ? callback() : callback(error)
                    );
            }
        });
        nextStream.map = (func) => arr.push({func: func});
        nextStream.filter = (func) => arr.push({filter: 1, func: func});

        return this.pipe(nextStream);
    }
