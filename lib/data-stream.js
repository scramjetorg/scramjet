...
module.exports = {
     ...
     * @deprecated use {@link ScramjetCore~DataStream#each} instead
     *
     * @memberof module:ScramjetCore~DataStream#
    ...
    join(item) {
        ...
        if (typeof item !== 'function') {
            const joint = item;
            item = (prev, next) => prev && next ? joint : null;
        }
        return this.shift(1, first => prev = first)
            .flatMap(
                next => Promise.resolve(item(prev, next))
                .then(joint => (prev = next, [joint, next]))
            );
    },
    ...
    toJSONArray(enclosure = '[]') {
        return this.stringify(JSON.stringify)
            .join(',')
            .unshift(enclosure[0])
            .endWith(enclosure[1]);
    },
    ...
    toJSONObject(entryCallback, enclosure = '{}') {
        ...
        return ref
            .stringify(([key, value]) => JSON.stringify(key.toString()) + ':' + JSON.stringify(value))
            .join(',')
            .unshift(enclosure[0])
            .endWith(enclosure[1]);
    },
