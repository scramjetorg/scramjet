...
module.exports = {
     ...
     *
     * @param  {Boolean|String} endline whether to add endlines (boolean or string as delimiter)
     * @return {StringStream}  output stream
     *
     * @example {@link ../samples/data-stream-jsonstringify.js}
     */
     ...
     *
     * @example {@link ../samples/data-stream-csvstringify.js}
     */
    CSVStringify(options) {
        const csv = require("csv-stringify");
        return this.pipe(csv(options)).pipe(new StringStream());
    },

    /**
     * Distributes processing into multiple subprocesses or threads if you like.
     *
     * @todo Make sure we keep order
     * @todo use fini to compare and mark item order
     * @todo allow passing serialize/deserialize methods for child_process
     * @todo doesn't work
     ...
     * @param {String[]}  [plugins=[]] List of scramjet plugin module paths
     * @chainable
     */
    distribute(affinity, clusterFunc, {plugins = [], options = {}} = {}) {    // eslint-disable-line
        return this.separate(affinity, options.createOptions, this.constructor)
            .cluster(clusterFunc, {
                plugins,
    ...
    separateInto(streams, affinity) {
        this.consume(
            async (chunk) => {
                ...
                throw new Error('Output for ' + streamId + ' not found');
            }
        );
        return this;
    },
