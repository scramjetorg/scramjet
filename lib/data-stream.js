...
class DataStream extends PromiseTransformStream {
     ...
     * @param  {GroupCallback} hashFunc the callback function
     * @param  {TeeCallback} streamFunc the callback function
     * @return {DataStream}  the re-muxed stream
     *
     * @example {@link ../samples/data-stream-group.js}
     */
    group(hashFunc, streamFunc, ...distrubuteArgs) {
        /* TODO: how to keep order - need that keep/release? */
        return this.separate(hashFunc, streamFunc, ...distrubuteArgs).mux();
    }
     ...
     * @param  {GroupCallback} hashFunc the callback function
     * @param  {TeeCallback} streamFunc the callback function
     * @return {MultiStream}  the splitted MultiStream
     *
     * @example {@link ../samples/data-stream-group.js}
     */
    separate(hashFunc, streamFunc, ...distrubuteArgs) {
        const multi = new scramjet.MultiStream().distribute(...distrubuteArgs);
        const map = new Map();

        this.map(
            (item) => Promise.all([
                Promise.resolve(item).then(hashFunc),
                item
            ])
        ).accumulate(
            /* TODO: how to keep backpressure here? */
            (tuple) => {
                if (!map.has(tuple[0])) {
                    const newStream = this._selfInstance();
                    map.put(tuple[0], newStream);
                    multi.add(streamFunc(newStream));
                }

                const out = map.get(tuple[0]);
                return out.write() || new Promise((res) => out.on("drain", res));
            }
        );
    ...
    /**
     * Keeps items for later release in future after a number of operations
     */
    keep() {
    ...
    /**
     * Releases a kept items in a tuple with tranformed items
     */
    release() {
    ...
    accumulate(func, into) {
        return new Promise((res, rej) => {
            this.pipe(new PromiseTransformStream({
                    parallelTransform: func.bind(null, into),
                    referrer: this
                }).tap())
                .on("finish", () => res(into))
                .on("error", rej);
        ...
        const ref = this.pipe(new DataStream({
            parallelTransform(chunk) {
                ...
                return last = last.then(
                    () => func(
                        (newChunk) => pushed = this.push(newChunk) && pushed,
                        chunk
                    )
                ).then(
                    () => pushed || new Promise((s) => this.once("drain", s))
                );
            },
            referrer: this
        }).tap());
    ...
    filter(func) {
        return this.pipe(this._selfInstance({
            parallelTransform: func,
            referrer: this,
            afterTransform: (ret, chunk) => ret ? chunk : null
        }));
    }
