...
module.exports = {
    ...
    exec(cmd, options, ...args) {
        const { stream = 1, interpreter = platform === "win32" ? ["cmd.exe","/c"] : [] } = (options || {});
        ...
        const actualCmd = interpreter && interpreter.length ? interpreter[0] : args;

        console.log(actualCmd, ...actualArgs);
