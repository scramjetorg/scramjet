const {DataStream, errors: {StreamError}} = require("./");
...
module.exports = {
    ...
    exec(cmd, options, ...args) {
        ...
        const cwd = getCalleeDirname(1);

        const spawnOptions = Object.assign({cwd}, options);
        spawnOptions.stdio = ["pipe", "pipe", "pipe"];

        const actualArgs = interpreter && interpreter.length ? [...interpreter.slice(1), cmd, ...args] : args;
        const actualCmd = interpreter && interpreter.length ? interpreter[0] : cmd;
        ...
        proc.on("exit", async code => {
            if (code > 0) {
                const error = new Error(`Non-zero exitcode (${code}) returned from command "${cmd}"`);
                error.exitCode = code;
