...
module.exports = {
    ...
    exec(cmd, options, ...args) {
        ...
        const resolvedCmd = path.resolve(getCalleeDirname(1), cmd);

        const actualArgs = interpreter && interpreter.length ? [...interpreter.slice(1), resolvedCmd, ...args] : args;
        const actualCmd = interpreter && interpreter.length ? interpreter[0] : resolvedCmd;
        ...
        proc.on("exit", async code => {
            if (code > 0) {
                const error = new Error(`Non-zero exitcode (${code}) returned from command "${resolvedCmd}"`);
                error.exitCode = code;
